...
int count_lef_nodes(Node* root)
{
    if(root == NULL) return 0; // If the tree is empty, return 0
    
    if(root->left == NULL && root->right == NULL) return 1; // If it's a leaf node, return 1
    int l = count_lef_nodes
    (root->left); // Count nodes in the left subtree
    int r = count_lef_nodes(root->right); // Count nodes in the right subtree
    return l + r ; // Return the total count (left + right + 1 for the current node)
}
int main()
{
    Node* root = input_tree(); // Input the binary tree structure
    
    cout << count_lef_nodes(root) << endl; // Output the count of nodes in the binary tree
    return 0;
}